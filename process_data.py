...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build story.json and processed image folders from raw construction assets."
    )
    parser.add_argument("--work-dir", default=".", help="Working directory for relative paths.")
    parser.add_argument("--vidpair-dir", default="VidPair-Halluc", help="Directory containing per-id video folders.")
    parser.add_argument("--raw-data-dir", default="raw_data", help="Directory containing raw per-category JSON/image assets.")
    parser.add_argument("--processed-dir", default="processed_data", help="Output directory for processed images.")
    parser.add_argument("--id-output", default="id.txt", help="Output text file for sorted ids.")
    parser.add_argument("--story-output", default="story.json", help="Output story JSON path.")
...
def find_json_file(category, raw_data_dir):
    """查找对应类别的JSON文件"""
    possible_names = [
        f"{category}.json",
    ...
    for name in possible_names:
        json_path = os.path.join(raw_data_dir, category, name)
...
def main():
    ...
    print(f"ID列表已保存到 {args.id_output}")
    print("前10个ID:")
    ...
    for i, id_name in enumerate(id_list, 1):
        ...
        if category == "sequence":
            ...
            value1_pattern = os.path.join(args.raw_data_dir, category, f"{id_name}_*_value_1.png")
            ...
            main_pattern = os.path.join(args.raw_data_dir, category, f"{id_name}_segment_1_*.png")
            ...
            results_dir = os.path.join(args.raw_data_dir, category, "results")
        ...
        # 3. 从JSON文件中提取对应的数据
        json_file = find_json_file(category, args.raw_data_dir)
        found_data = None
    ...
    with open(args.story_output, "w", encoding="utf-8") as f:
        json.dump(story_data, f, ensure_ascii=False, indent=2)
    
    print("")
    print("数据处理完成！")
    print(f"处理了 {len(id_list)} 个ID")
    print(f"图片已复制到 {args.processed_dir}/ 目录")
    print(f"JSON数据已保存到 {args.story_output} (包含 {len(story_data)} 个条目)")
    print("")
